...
<div style="background: #FDFEFE;">
  <h4> <br><b>STEP 3 :</b> Choose the territories you want to keep for this year <?php
  print($last_year+1);
  ?>, for this mark the boxes needed of the last column of the tab <b>AND</b> click on "add territory to "<?php
  print($last_year);
  ?>" button .<br><br></h4>
</div>
...
<div>
    ...
    for ($i=0;$i<count($id);$i++){
        echo('<tr><td>'.$id[$i].'</td><td>'.$field_id[$i].'</td><td>'.$burrow_nb[$i].'</td><td>'.$aspect[$i].'</td><td>'.$surface[$i].'</td><td>'.$vegetation[$i].'</td><td>'.$snowmelt_date[$i].'</td><td>'.$comment[$i].'</td><td>'.$study_territory[$study_site_id[$i]].'</td><td>'.$latitude[$i].'</td><td>'.$longitude[$i].'</td><td>'.$altitude[$i].'</td><td><input type="checkbox" name="choice[]" value="'.$id[$i].'">Add to the year'); // Display of an handler with the checkbox to add it in a year
    }
    ?>
    </tbody>
    </table>
...
<select class="custom-select" id="disabledSelect" style="display:none" name="year">
    ...
    <?php
        print("<input type='submit' class='btn btn-success btn-lg btn-block' value='add territory to $current_year' name='add' method='post'>");
        ?>

</form>

<div style="background: #FDFEFE;">
  <h4> <br><b>STEP 4 :</b> On the grey table below you'll find all the territory of the study site of interest present in the database for <?php print($last_year+1); ?>. If you need to add new territory click on the « add a new Territoy » button.<br><br></h4>
</div>
<div>
...
<div>
    <button type="button" class="btn btn-success btn-lg btn-block" data-toggle="modal" data-target="#exampleModal2">add new Territory</button>
<br>
</div>
</div>
