...
<div style="background: #FDFEFE;">
  <h4> <br> On this page you’ll be able to fill up the handler and handler year table for year <?php
      ...
      ?>
        in 2 steps. <br></h4>
  <h4> <br><b>STEP 1 :</b> You’ll find all the handler present in the database for <?php print($last_year); ?> in the table below.  <br>
    Choose the handlers you want to keep for this year (<?php print($current_year); ?>), for this mark the boxes needed of the last column of the tab <b>AND </b> click on « add handler to <?php print($current_year); ?> ».<br></h4>
</div>
</div>
<div>
<div>
      ...
      <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">New Handler</h5>
...
<div>
    ...
    for ($i=0;$i<count($id);$i++){ // to display the table rows
        echo('<tr><td>'.$id[$i].'</td><td>'.$last_name[$i].'</td><td>'.$first_name[$i].'</td><td>'.$status[$i].'</td><td>'.$comment[$i].'</td><td>'.$provider_name[$i].'</td><td>'.$email[$i].'</td><td><input type="checkbox" name="choice[]" value="'.$id[$i].'">Add to the year'); // Display of an handler with the checkbox to add it in a year
    }
    ?>
    </tbody>
    </table>
...
<div style="background: #FDFEFE;">
   <br>  <br>
  <h4> <br><b>STEP 2 :</b>On the grey table below you'll find all the handler present in the database for <?php print($last_year+1); ?>. If you need to add new handler click on the « add a new handler» button.<br></h4>
</div>
...
for ($i=0;$i<count($id_thisyear);$i++){
    echo('<tr><td>'.$id_thisyear[$i].'</td><td>'.$last_name_thisyear[$i].'</td><td>'.$first_name_thisyear[$i].'</td><td>'.$status_thisyear[$i].'</td><td>'.$comment_thisyear[$i].'</td><td>'.$provider_name_thisyear[$i].'</td><td>'.$email_thisyear[$i].'</td>'); // Display of an handler with the checkbox to add it in a year
}
?>
</tbody>
</table>
