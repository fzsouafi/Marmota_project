...
<div style="background: #FDFEFE;">
  <h1> Study and Territory Site Form </h1>
</div>
<div style="background: #FDFEFE;">
  <h4> <br> On this page you’ll be able to fill up the study and territory site table for year <?php
      print($last_year+1);
      ?>
        in 4 steps. <br></h4>
  <h4> <br><b>STEP 1 :</b> You’ll find all the study site present in the database in the table below. <br>
    To add a new one, click on the « add a new Study site » button.<br></h4>
</div>
...
<div>
    ...
    for ($i=0;$i<count($id);$i++){
        if ($_GET['multiform']==true){
            echo("<tr><td>$id[$i]</td><td><a href='index.php?page=formAddStudySite&amp;study_site=$id[$i]&amp;year=$current_year&amp;multiform=true'>$name[$i]</a></td><td>$description[$i]</td><td>$comment[$i]</td><td>$map[$i]</td><td>$latitude[$i]</td><td>$longitude[$i]</td><td>$altitude[$i]</td></tr>"); // Display of a study_site with the link to get the territory associated for multiform
        }
        else{
            echo("<tr><td>$id[$i]</td><td><a href='index.php?page=formAddStudySite&amp;study_site=$id[$i]&amp;year=$current_year'>$name[$i]</a></td><td>$description[$i]</td><td>$comment[$i]</td><td>$map[$i]</td><td>$latitude[$i]</td><td>$longitude[$i]</td><td>$altitude[$i]</td></tr>"); // Display of a study_site with the link to get the territory associated without multiform
        }
    }
    ?>
    </tbody>
...
<div style="background: #FDFEFE;">
  <h4> <br><b>STEP 2 :</b> Click on the name of the study site of interest (in the tab above) to see his territories of <?php
  print($last_year);
  ?>
    in a table below.<br><br></h4>
</div>
